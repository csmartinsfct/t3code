...
// ---------------------------------------------------------------------------
// Separator badge variant resolution
// ---------------------------------------------------------------------------
...
const TimelineSection = memo(function TimelineSection({
  ...
  const canNavigate = section.threadId && onNavigateToThread;
  let reviewCardCount = 0;

  // Derive review label from activity outcome
  const sectionLabel = isReview
    ? section.reviewOutcome === "approved"
      ? "Review Passed"
      : section.reviewOutcome === "requested-changes"
        ? "Review Failed"
        : section.reviewOutcome === "blocked"
          ? "Review Blocked"
          : "Review"
    : "Implementation";
  ...
  return (
    <div>
      {/* Section divider — clickable to navigate to thread */}
      {canNavigate ? (
        <button
          type="button"
      ...
      {section.messages.length === 0 ? (
        ...
        <div className="space-y-3">
          {section.messages.map((message) => {
            // Hide review user prompts — they add noise
