...
export type ComposerSlashCommand = "model" | "plan" | "plan-accept" | "default";
...
const SLASH_COMMANDS: readonly ComposerSlashCommand[] = ["model", "plan", "plan-accept", "default"];
...
export function parseStandaloneComposerSlashCommand(
  ...
  const match = /^\/(plan-accept|plan|default)\s*$/i.exec(text.trim());
  if (!match) {
    return null;
  }
  const command = match[1]?.toLowerCase();
  if (command === "plan-accept") return "plan-accept";
