import { isValidProviderKind, type ProviderKind, type ThreadId } from "@t3tools/contracts";
...
function decodeProviderKind(
  ...
  if (isValidProviderKind(providerName)) {
...
function makeProviderSessionDirectoryImpl(options?: {
  lifecycleLogger?: ProviderLifecycleLoggerShape;
}) {
...
const makeProviderSessionDirectory = makeProviderSessionDirectoryImpl();
...
export function makeProviderSessionDirectoryLive(options?: {
  lifecycleLogger?: ProviderLifecycleLoggerShape;
}) {
  return Layer.effect(ProviderSessionDirectory, makeProviderSessionDirectoryImpl(options));
}
