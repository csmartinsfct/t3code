...
import { ChatAttachment, OrchestrationMessageMetadata } from "@t3tools/contracts";
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextMetadataJson = row.metadata !== undefined ? JSON.stringify(row.metadata) : null;
      return sql`
  ...
  const deleteProjectionThreadMessageRowsByIds = (
    input: DeleteProjectionThreadMessagesByIdsInput,
  ) => {
    if (input.messageIds.length === 0) return Effect.void;
    return sql`
      DELETE FROM projection_thread_messages
