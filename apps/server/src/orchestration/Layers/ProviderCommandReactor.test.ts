...
describe("ProviderCommandReactor", () => {
  ...
  it("generates a thread title on the first turn", async () => {
    ...
    await waitFor(async () => {
      const readModel = (await Effect.runPromise(harness.engine.getReadModel())) as any;
      return (
        readModel.threads.find((entry: any) => entry.id === ThreadId.makeUnsafe("thread-1"))
  ...
  it("restarts the provider session when a same-base provider profile changes", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-profile-1"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-profile-1"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-profile-2"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-profile-2"),
          role: "user",
          text: "second",
          attachments: [],
        },
  ...
  it("restarts the provider session when the thread cwd changes", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-cwd-1"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-cwd-1"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.makeUnsafe("cmd-thread-meta-update-cwd"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        branch: "t3code/repo-local-config",
        worktreePath: "/tmp/provider-project-worktree",
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-cwd-2"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-cwd-2"),
          role: "user",
          text: "second",
          attachments: [],
        },
  ...
  it("surfaces stale provider approval request failures without faking approval resolution", async () => {
    ...
    await waitFor(async () => {
      ...
      return thread.activities.some(
        (activity: any) => activity.kind === "provider.approval.respond.failed",
      );
    });
  ...
  it("surfaces stale provider user-input failures without faking user-input resolution", async () => {
    ...
    await waitFor(async () => {
      ...
      if (!thread) return false;
      return thread.activities.some(
        (activity: any) => activity.kind === "provider.user-input.respond.failed",
      );
    });
  ...
  it("still clears provider bindings when thread.session.stop is requested for an already stopped thread", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.makeUnsafe("cmd-session-set-stopped-for-stop"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        session: {
          threadId: ThreadId.makeUnsafe("thread-1"),
          status: "stopped",
          ...
          runtimeMode: "approval-required",
          activeTurnId: null,
          lastError: "stale provider binding",
          updatedAt: now,
        },
        createdAt: now,
      }),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.stop",
        commandId: CommandId.makeUnsafe("cmd-session-stop-stopped-thread"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        createdAt: now,
      }),
    );
