  ...
  function* () {
      ...
      switch (event.type) {
        ...
        case "project.meta-updated": {
          ...
          yield* projectionProjectRepository.upsert({
            ...
            ...(event.payload.nameHidden !== undefined
              ? { nameHidden: event.payload.nameHidden }
              : {}),
      ...
      switch (event.type) {
        case "thread.created":
          yield* projectionThreadRepository.upsert({
            ...
            initialDraft: event.payload.initialDraft ?? null,
            latestTurnId: null,
            createdAt: event.payload.createdAt,
            updatedAt: event.payload.updatedAt,
            archivedAt: null,
        ...
        case "thread.moved": {
          ...
          yield* projectionThreadRepository.upsert({
            ...existingRow.value,
            projectId: event.payload.targetProjectId,
            branch: null,
            worktreePath: null,
            updatedAt: event.payload.updatedAt,
          });
      ...
      switch (event.type) {
        case "thread.proposed-plan-upserted":
          yield* projectionThreadProposedPlanRepository.upsert({
            ...
            status: event.payload.proposedPlan.status,
    ...
    const applyThreadTurnsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.session-set": {
          const completedTurn = event.payload.completedTurn;
          if (completedTurn) {
            ...
            if (Option.isSome(existingTurn)) {
              yield* projectionTurnRepository.upsertByTurnId({
                ...
                startedAt: existingTurn.value.startedAt ?? completedTurn.completedAt,
                requestedAt: existingTurn.value.requestedAt ?? completedTurn.completedAt,
                completedAt: completedTurn.completedAt,
                terminalReason: completedTurn.terminalReason ?? null,
              });
        ...
        case "thread.turn-interrupt-requested": {
          ...
          if (Option.isSome(existingTurn)) {
            yield* projectionTurnRepository.upsertByTurnId({
              ...
              startedAt: existingTurn.value.startedAt ?? event.payload.createdAt,
              requestedAt: existingTurn.value.requestedAt ?? event.payload.createdAt,
              terminalReason: existingTurn.value.terminalReason ?? "aborted_streaming",
            });
            return;
          }
          yield* projectionTurnRepository.upsertByTurnId({
        ...
        case "thread.turn-diff-completed": {
          ...
          if (Option.isSome(existingTurn)) {
            yield* projectionTurnRepository.upsertByTurnId({
              ...
              terminalReason: existingTurn.value.terminalReason,
            });
            return;
          }
          yield* projectionTurnRepository.upsertByTurnId({
