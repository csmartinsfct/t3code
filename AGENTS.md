...
- [Overlay Surfaces](docs/overlays.md) — UI/runtime contract for menus, popovers, dialogs, sheets, command dialogs, and native overlay `WebContentsView` fallback behavior when the embedded browser is visible. When changing overlay path selection, wrapper semantics, dismissal behavior, or browser-visible fallback behavior, update this doc first; then update Browser Tools and feature docs for implementation/example details.
