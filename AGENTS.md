...
T3 Code is a minimal web GUI for using coding agents like Codex, Claude, Gemini, and Cursor.
...
- `apps/server`: Node.js WebSocket server. Wraps provider processes (Codex app-server, Claude Agent SDK, Gemini ACP, Cursor ACP), serves the React web app, and manages provider sessions.
...
Codex support starts `codex app-server` (JSON-RPC over stdio) for Codex sessions, then streams structured events to the browser through WebSocket push messages. Claude, Gemini, and Cursor use their own provider adapters but flow through the same provider/orchestration pipeline.
...
- [Browser Tools](docs/browser-tools.md) — Per-project browser automation via `/api/browser`. Two hosts (Playwright for headless / CI, Electron `WebContentsView` + `webContents.debugger` CDP for the embedded desktop UI), `host.json` authoritative-mode, `@ref` element IDs, 58 tools, cross-process wiring + sequence diagrams, vendored GStack Browser, extension support, known issues.
- [Browser Extensions](docs/browser-extensions.md) — Chrome extension support: install from Web Store, the `electron-chrome-extensions` fork (why we forked, gaps fixed, build instructions), background polyfill, NSPanel native addon, extensions panel UI, popup windows, dapp approval flow, and open tickets.
- [Browser Transport Decision](docs/browser-transport-decision.md) — Why the embedded-browser CDP bridge uses an Electron-main-owned loopback HTTP broker with a bearer token rather than fd framing or `utilityProcess`. Read before touching `CdpBroker`, `ElectronCdpHttpTransport`, or the bootstrap envelope.
- [Overlay Surfaces](docs/overlays.md) — UI/runtime contract for menus, popovers, dialogs, sheets, command dialogs, and native overlay `WebContentsView` fallback behavior when the embedded browser is visible. Browser-visible native overlays use routed/shared-JSX overlays; do not add serialized primitive/JSON overlay renderers. When changing overlay path selection, wrapper semantics, dismissal behavior, or browser-visible fallback behavior, update this doc first; then update Browser Tools and feature docs for implementation/example details, and remove any dead overlay code/docs from older paths.
...
- [Cursor Provider](docs/cursor-provider-implementation.md) — Cursor ACP integration, model/mode dispatch, MCP approvals/login, attachments, plan handling, profile settings, and verified CLI findings.
