...
describe("dynamic chat UI language detection", () => {
  ...
  it("rejects unrelated languages", () => {
    expect(isDynamicChatUiLanguage("json")).toBe(false);
...
describe("isDynamicChatUiStatusBlock", () => {
  ...
  it("rejects artifact and unrelated languages", () => {
    expect(isDynamicChatUiStatusBlock("language-t3:dynamic-chat-ui")).toBe(false);
...
describe("parseDynamicChatUiStatusPayload", () => {
    ...
    expect(
      parseDynamicChatUiStatusPayload(
        JSON.stringify({
          ...
          description: "Creating the dashboard.",
          state: "generating",
        }),
      ),
    ).toEqual({
  ...
  it("rejects malformed or completed status payloads", () => {
    expect(parseDynamicChatUiStatusPayload("{")).toBeNull();
    expect(parseDynamicChatUiStatusPayload("{}")).toBeNull();
    expect(
      parseDynamicChatUiStatusPayload(
        JSON.stringify({ version: 1, title: "Done", state: "complete" }),
      ),
    ).toBeNull();
  });
});

describe("extractDynamicChatUiArtifactsFromMarkdown", () => {
  it("extracts durable artifact documents from legacy markdown blocks", () => {
    const markdown = [
    ...
    const artifacts = extractDynamicChatUiArtifactsFromMarkdown(markdown);
    ...
    expect(extractDynamicChatUiPayloadsFromMarkdown(markdown)).toMatchObject([
      {
        id: "scenario-sim",
        html: '<section><input type="range" /></section>',
      },
  ...
  it("ignores invalid or unrelated fences", () => {
    const artifacts = extractDynamicChatUiArtifactsFromMarkdown(
      ["```json", "{}", "```", "```t3-dynamic-chat-ui", "{", "```"].join("\n"),
    );
...
describe("stripDynamicChatUiFencesFromMarkdown", () => {
  it("removes only dynamic chat UI fences from display text", () => {
    const markdown = [
      "Intro",
      "```t3:dynamic-chat-ui",
      JSON.stringify({ version: 1, html: "<div />" }),
      "```",
      "```ts",
      "const kept = true;",
      "```",
    ...
    expect(stripDynamicChatUiFencesFromMarkdown(markdown)).toBe(
      ["Intro", "", "```ts", "const kept = true;", "```", "Outro"].join("\n").trim(),
    );
  });
});
