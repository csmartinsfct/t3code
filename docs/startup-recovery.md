...
The web store treats hydrated thread content as a bounded client-side cache. Loaded messages, activities, proposed plans, and checkpoint summaries are tracked with an approximate byte size and LRU access time. When the cache exceeds its byte budget, unprotected loaded threads are evicted back to metadata-only state (`messages: "not-loaded"` plus empty heavyweight arrays); the next route visit rehydrates from SQLite through `orchestration.getThreadContent`.

Eviction is constrained by UI and runtime state. The focused chat route, orchestration parent/child threads that are visible in the current route, and threads with active provider sessions are pinned and skipped by eviction. Route components report those visible IDs to the store, keeping React Router coupling at the boundary instead of spreading route awareness through the cache policy.

Reconnect and in-flight hydration both follow the same sequence contract. Domain events for unloaded or evicted threads still update metadata immediately and are retained as lightweight pending events. When content hydration completes, the store applies only pending events with `event.sequence > content.sequence`, so events that arrived after the content snapshot are restored without duplicating entries already present in the hydrated SQLite projection. Replay events received after reconnect can therefore update evicted threads as metadata-only; navigation later rehydrates the content and reconciles any retained post-snapshot events.
