...
The orchestration planner uses dependencies to order included tickets when both sides of a dependency are in the run. Unfinished dependencies outside the run are shown as warnings only; they do not block starting orchestration. Dependencies inherited from parent tickets are reported against the parent ticket that owns the dependency rather than repeated on every descendant execution leaf.
