...
Action menus that require exact parity, including the Git actions menu, project-script actions menu, and plan action menus, use routed menu overlays rather than serialized `overlayItems`. Their DOM path and native route share a single content component; the route should only adapt transport concerns such as result submission, anchor positioning, and overlay dismissal.
