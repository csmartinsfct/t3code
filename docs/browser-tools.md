...
The bundled cache is version-specific to T3's packaged Playwright dependency; it is not a universal browser cache for checked-out repositories. Packaged provider and terminal processes inherit `PLAYWRIGHT_BROWSERS_PATH`, so a repository-owned Playwright, Vitest browser, or Storybook command may otherwise search T3's bundle for the repository's different browser revision. The production Electron browser prompt tells agents to remove the variable from those child commands (`env -u PLAYWRIGHT_BROWSERS_PATH <test-command>` on macOS/Linux, or the shell-appropriate Windows equivalent). Agents must not install repository browser revisions into the application bundle. An absent _different_ revision under `Resources/playwright-browsers/` is a cache/version mismatch, not evidence that the production bundle is missing.
...
| Symptom                                                                                 | Check                                                                                                                                                                                                                                                                         |
| --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Repository test requests an absent revision under T3's `Resources/playwright-browsers/` | This is usually a project/T3 Playwright revision mismatch. Remove `PLAYWRIGHT_BROWSERS_PATH` from the repository test command so Playwright uses the project's normal cache. Do not install the requested revision into T3's application bundle. See "Chromium bundle" above. |
| T3 `/api/browser` reports `Executable doesn't exist at .../chromium/chrome`             | Dev: run `bunx playwright install chromium` once per machine. Packaged app: verify the revision requested by T3's packaged Playwright exists in `Resources/playwright-browsers/`; if that matching revision is missing, the build was incomplete — reinstall the app.         |
...
`ContextWindowMeter` and `RateLimitMeter` route their rich hover popovers through the shared routed-popover helper in `apps/web/src/routedPopover.ts`. The trigger stays in the host DOM, passes a trigger rect plus a serializable snapshot to the overlay route, and both DOM fallback and native overlay render the exact same breakdown/chart content components. The Codex reset-credit section is part of that shared JSX: it shows the authoritative available count, the soonest expiration when detailed credit data is present, and a small `Use reset` action. In the native route, that action emits a non-dismissing event to the host; the host owns the idempotent WebSocket mutation and refreshes the serializable snapshot without closing the popover.
