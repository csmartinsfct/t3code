...
- For single-service composite runs the sub-strip is hidden and the viewport binds directly to that one service's stream. Legacy single-process runs use the synthetic `main` stream internally and persist log records with `serviceId: null`, so the viewport reads the merged run stream — no merged-view prefix.
